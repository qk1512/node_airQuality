...
bool SMisConnected()
{
    ...
    RS485.Rs485Modbus->Send(0x1E, SM_FUNCTION_CODE, (0x01 << 8 ) | 0x00 , 0x01);

    delay(200);
    
    //bool check_receive = 
    RS485.Rs485Modbus -> ReceiveReady();
    //AddLog(LOG_LEVEL_INFO, PSTR("Check receive: %d"), check_receive);
    uint8_t buffer[8];
    ...
    else
    {
        ...
        //AddLog(LOG_LEVEL_INFO, PSTR("SM Address: %u"), buffer[4]);
...
void SMInit(void)
{
    ...
    if(!SM_sensor.valid) 
    {
        ...
        //AddLog(LOG_LEVEL_INFO, PSTR("RS485 Active : "))
        //count_restart++;
    }
