...
#endif // USE_MQTT_FILE
    D_CMND_MQTTHOST "|" D_CMND_MQTTPORT "|" D_CMND_MQTTRETRY "|" D_CMND_MQTTCLIENT "|" D_CMND_FULLTOPIC "|" D_CMND_PREFIX "|" D_CMND_GROUPTOPIC "|" D_CMND_TOPIC "|" D_CMND_PUBLISH "|" D_CMND_MQTTLOG "|" D_CMND_BUTTONTOPIC "|" D_CMND_SWITCHTOPIC "|" D_CMND_BUTTONRETAIN "|" D_CMND_SWITCHRETAIN "|" D_CMND_POWERRETAIN "|" D_CMND_SENSORRETAIN "|" D_CMND_INFORETAIN "|" D_CMND_STATERETAIN "|" D_CMND_STATUSRETAIN
...
void MqttInit(void)
{
  ...
  { // look for ".iot." and ".amazonaws.com" in the domain name
...
void ProvisionAzureDPS()
{
  ...
  if (assigned)
  {
    ...
    if (newProvision)
    { // because this is the first time we have been provisioned must reboot
...
bool MqttPublishLib(const char *topic, const uint8_t *payload, unsigned int plength, bool retained)
{
...
#ifdef USE_TASMESH
  if (MESHrouteMQTTtoMESH(topic, (char *)payload, retained))
  { // If we are a node, send this via ESP-Now
    yield();
    return true;
  }
#endif // USE_TASMESH
  ...
  if (!message_object.isValid())
  { // only sending valid JSON, yet this is optional
  ...
  if (!payloadRoot)
  { // ignore any message without json format payload
    return true;
  }
...
void MqttDataHandler(char *mqtt_topic, uint8_t *mqtt_data, unsigned int data_len)
{
  ...
  char requestID[10];
  if (fullTopicString.startsWith("v1/devices/me/rpc/request/"))
  { // received server side RPC
...
void MqttPublishLoggingAsync(bool refresh)
{
  ...
  if (!Settings->mqttlog_level || !Settings->flag.mqtt_enabled || !Mqtt.connected)
  {
    return;
  } // SetOption3 - Enable MQTT
  if (refresh && !NeedLogRefresh(Settings->mqttlog_level, index))
  {
    return;
  }
...
void MqttPublishPayload(const char *topic, const char *payload, uint32_t binary_length, bool retained)
{
  ...
  {                   // SetOption104 - Disable all MQTT retained messages, some brokers don't support it: AWS IoT, Losant
  ...
  if (Settings->flag.mqtt_enabled && MqttPublishLib(topic, (const uint8_t *)payload, binary_length, retained))
  { // SetOption3 - Enable MQTT
#ifdef USE_TASMESH
...
void MqttPublishPowerState(uint32_t device)
{
  ...
  else
  {
    ...
    { // SetOption90 - Disable non-json MQTT response
  ...
  if (Mqtt.allowed)
  {
    ...
    if (strstr_P(SettingsText(SET_MQTT_FULLTOPIC), MQTT_TOKEN_TOPIC) != nullptr)
    {
      ...
      for (uint32_t i = 0; i < MAX_GROUP_TOPICS; i++)
      {
        if (1 == i)
        {
          real_index = SET_MQTT_GRP_TOPIC2 - 1;
        }
...
void MqttReconnect(void)
{
  ...
  if (Mqtt.allowed)
  {
    ...
    if (Mqtt.mqtt_tls)
    {
      if (0 == strlen(SettingsText(SET_MQTT_PWD)))
      { // we anticipate that an empty password does not make sense with TLS. This avoids failed connections
        Mqtt.allowed = false;
      }
    }
#endif
  ...
  String azureMqtt_userString = String(SettingsText(SET_MQTT_HOST)) + "/" + String(SettingsText(SET_MQTT_CLIENT));
  ...
  if (MqttClient.connect(TasmotaGlobal.mqtt_client,
                         ...
                         Settings->flag5.mqtt_persistent ? 0 : 1))
  { // Clean Session
#ifdef USE_MQTT_TLS
    if (Mqtt.mqtt_tls)
    {
...
void CmndMqttUser(void)
{
  ...
  {
    SettingsUpdateText(SET_MQTT_USER, (SC_CLEAR == Shortcut()) ? "" : (SC_DEFAULT == Shortcut()) ? PSTR(MQTT_USER)
                                                                                                 : XdrvMailbox.data);
    TasmotaGlobal.restart_flag = 2;
  }
...
void CmndMqttPassword(void)
{
  ...
  if (XdrvMailbox.data_len > 0)
  {
    SettingsUpdateText(SET_MQTT_PWD, (SC_CLEAR == Shortcut()) ? "" : (SC_DEFAULT == Shortcut()) ? PSTR(MQTT_PASS)
                                                                                                : XdrvMailbox.data);
    if (!show_asterisk)
    {
...
void CmndMqttHost(void)
{
  if (XdrvMailbox.data_len > 0)
  {
    SettingsUpdateText(SET_MQTT_HOST, (SC_CLEAR == Shortcut()) ? "" : (SC_DEFAULT == Shortcut()) ? MQTT_HOST
                                                                                                 : XdrvMailbox.data);
    TasmotaGlobal.restart_flag = 2;
  }
    ...
    else
    {
      if (XdrvMailbox.data_len > 0)
      {
        ...
        SettingsUpdateText(SET_MQTTPREFIX1 + XdrvMailbox.index - 1,
                           (SC_DEFAULT == Shortcut()) ? (1 == XdrvMailbox.index) ? PSTR(SUB_PREFIX) : (2 == XdrvMailbox.index) ? PSTR(PUB_PREFIX)
                                                                                                                               : PSTR(PUB_PREFIX2)
                                                      : XdrvMailbox.data);
        TasmotaGlobal.restart_flag = 2;
      }
...
void CmndGroupTopic(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_GROUP_TOPICS))
  {
    if (XdrvMailbox.data_len > 0)
    {
      ...
      SettingsUpdateText(settings_text_index, (SC_CLEAR == Shortcut()) ? "" : (SC_DEFAULT == Shortcut()) ? PSTR(MQTT_GRPTOPIC)
                                                                                                         : XdrvMailbox.data);
...
void CmndButtonTopic(void)
{
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0))
  {
    ...
    case SC_CLEAR:
      ...
      break;
    case SC_DEFAULT:
      SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, TasmotaGlobal.mqtt_topic);
      break;
    case SC_USER:
...
void CmndSwitchTopic(void)
{
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0))
  {
    ...
    case SC_CLEAR:
      ...
      break;
    case SC_DEFAULT:
      SettingsUpdateText(SET_MQTT_SWITCH_TOPIC, TasmotaGlobal.mqtt_topic);
      break;
    case SC_USER:
...
void CmndInfoRetain(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 1))
  {
    if (!XdrvMailbox.payload)
    {
      ...
      { // Remove retained INFO1, INFO2 and INFO3
...
void CmndStatusRetain(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 1))
  {
    if (!XdrvMailbox.payload)
    {
      ...
      char stemp1[10];
      for (uint32_t i = 0; i <= MAX_STATUS; i++)
      { // Remove retained STATUS0, STATUS1 .. STATUS13
...
void CmndTlsDump(void)
{
  ...
  {
    return;
  } // safeguard for ESP32, removed by compiler for ESP8266
